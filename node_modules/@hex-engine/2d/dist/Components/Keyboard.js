import { useEnableDisable, useType, useCallbackAsCurrent } from "@hex-engine/core";
import { Vector } from "../Models";
import { useContext } from "../Hooks";
var firstKeyHasHappened = false;
var pendingFirstKeyHandlers = [];
/**
...
export function useFirstKey(handler) {
...
export default function Keyboard() {
  ...
  var processKeydown = function processKeydown(event) {
    if (preventDefault) {
    ...
    if (!firstKeyHasHappened) {
      ...
      pendingFirstKeyHandlers.forEach(function (cb) {
        return cb();
      });
  ...
  var processKeyup = function processKeyup(event) {
    event.preventDefault();
  ...
  if (!doc) {
    throw new Error("Root canvas is not part of a document; therefore, Keyboard can't setup event listeners");
  }
  ...
  return {
    ...
    vectorFromKeys: function vectorFromKeys(upKey, downKey, leftKey, rightKey) {
      ...
      if (pressedKeys.has(rightKey) || pressedKeys.has(rightKey.toUpperCase())) {
        x += 1;
      }
