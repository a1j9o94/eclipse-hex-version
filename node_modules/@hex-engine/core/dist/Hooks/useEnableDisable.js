...
export function StorageForUseEnableDisable() {
  ...
  return {
    enableCallbacks: new WeakMap(),
    disableCallbacks: new WeakMap()
  };
}
/**
 *
...
export default function useEnableDisable() {
  var storage = useEntity().getComponent(StorageForUseEnableDisable) || useNewComponent(StorageForUseEnableDisable);
  ...
  return {
     ...
     */
    onEnabled: function onEnabled(handler) {
      var componentStorage = storage.enableCallbacks.get(component);

      if (!componentStorage) {
        componentStorage = new Set();
     ...
     */
    onDisabled: function onDisabled(handler) {
      var componentStorage = storage.disableCallbacks.get(component);
